...
\begin{definition}\label{regularsytemofhodgebundle}
    一个$X^{alg}$上的\textbf{正则丛}是一个代数Higgs丛$E$并存在以下滤链：
    \[
    j_*^{alg}E=\bigcup_{\alpha\in \R^{Y}} E_{\alpha}
    \]
其中$E_\alpha$皆为凝聚子层，我们赋予$\R^{Y}$自然的偏序结构。并且我们要求Higgs场满足
\[
\theta: E_\alpha \to E_\alpha\otimes \Omega_X^1\langle\log(Y)\rangle
\]
下面我们会看到这样的定义是自然的。假设我们有一个全纯的Higgs丛$E\to X$，如果$E$上存在一个在奇点附近行为较好的度量$h$，我们可以通过如下方式构造出一族子层$a_hE=\bigcup_{\alpha\in \R^{Y}} E_\alpha$：我们定义$E_\alpha$为$j^{an}_*E$当中满足以下条件的子层，对任意的$y\in Y$，假设$y\in\U$，我们只选取增长速度满足
\[
|s|_h\leq C r^{\alpha_y+\epsilon},\quad \forall\epsilon>0
\]
的那些截面$s$。

Simpson\cite{Si1}证明了以下定理，
\begin{theorem}
    假设$(E,\overline{\partial},\theta, h)$是一个带度量的抛物Higgs丛，并且度量$h$满足
    \[
    \int_X |F_{\nabla^{hs}_h}|^p <\infty,\quad p>1
    \]
    则如上定义的$E_\alpha$是凝聚层，并且$\theta$满足
    \[
    \theta: E_\alpha \to E_\alpha \otimes \Omega_X^2\langle\log Y\rangle
    \]
    因此$E$是一个正则丛。上述的构造 $a$ 和取$\det$，对偶，张量积可交换。假设
\end{theorem}


任给切向量$v\in T^{1,0}$，$\theta(v)$是周期域$G_0/K_0$的切向量，由\cite{griffiths1969locally}中的结论，周期域的全纯截面曲率在$\g^{-1,1}$方向上是严格负的，因此我们有
\[
